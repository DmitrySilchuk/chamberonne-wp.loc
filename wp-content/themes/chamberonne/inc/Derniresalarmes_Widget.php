...
// Adds widget: DERNIÈRES ALARMES
class Derniresalarmes_Widget extends WP_Widget {

    function __construct() {
        parent::__construct(
            'derniresalarmes_widget',
            esc_html__( 'DERNIÈRES ALARMES', 'chamberonne' )
        );
    }
...
HTML;
            foreach ($posts as $post) {
                ...
                $alarm_date = get_field('alarm_date', $post->ID );
                $alarm_date = date_create($alarm_date);
                $alarm_date = date_format($alarm_date, 'd.m.y');

                $number = get_field('alarm_number', $post->ID );
                $has_picture_icon = '';
...
function register_derniresalarmes_widget() {
    register_widget( 'Derniresalarmes_Widget' );
}
add_action( 'widgets_init', 'register_derniresalarmes_widget' );